...
function BenchmarkComparisonSection({
  ...
  if (!isPremium) {
    return (
      <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.hairline, overflow: "hidden" }]}>
        ...
        <View style={styles.premiumOverlay}>
          <TouchableOpacity
            style={[styles.premiumBadge, { backgroundColor: theme.accent + "22", borderColor: theme.accent + "44" }]}
            onPress={onUpgrade}
            activeOpacity={0.8}
          >
            <Feather name="lock" size={16} color={theme.accent} />
            <Text style={[styles.premiumText, { color: theme.accent }]}>Premium Feature</Text>
          </TouchableOpacity>
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
          ...
          {BENCHMARKS.map((bm) => (
            <TouchableOpacity
              ...
              style={[
                styles.benchChip,
                {
                  backgroundColor: activeBench.symbol === bm.symbol ? theme.surface : theme.surface,
                  borderColor: activeBench.symbol === bm.symbol ? theme.accent : theme.hairline,
                },
              ]}
              onPress={() => handleChipTap(bm)}
            >
              <Text style={[styles.benchChipText, { color: activeBench.symbol === bm.symbol ? theme.accent : theme.textSecondary }]}>
                {bm.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      {loading ? (
        ...
        <View style={[dStyles.summaryBox, { backgroundColor: theme.surface, borderRadius: 10 }]}>
          ...
          <View style={[dStyles.summaryDivider, { backgroundColor: theme.hairline }]} />
          <View style={dStyles.summaryRow}>
...
function PortfolioChart({
  ...
  return (
    <View style={{ width, height: CHART_H, position: "relative", overflow: "hidden" }}>
      {yLabels.map((lbl, i) => (
        <View key={i} style={[styles.gridLine, { top: lbl.y, left: PAD.left, right: PAD.right, borderColor: theme.hairline }]} />
      ))}
      {yLabels.map((lbl, i) => (
...
const SCENARIOS = [
  ...
  { label: "Base",         key: "base"         as const, pct: 7,  color: "#FBBF24" },
    ...
    <View>
      <Svg width={width} height={H}>
        {yLabels.map((l, i) => (
          <Line key={i} x1={PAD.left} y1={l.y} x2={PAD.left + iW} y2={l.y}
            stroke={theme.hairline} strokeWidth={1} strokeDasharray="4,4" />
        ))}
        {yLabels.map((l, i) => (
          <SvgText key={i} x={PAD.left - 4} y={l.y + 4} fontSize={9}
            fill={theme.textMuted} textAnchor="end" fontFamily="Archivo_400Regular">
            {l.label}
          </SvgText>
        ))}
        {xLabels.map((l, i) => (
          <SvgText key={i} x={l.x} y={H - 4} fontSize={9}
            fill={theme.textMuted} textAnchor="middle" fontFamily="Archivo_400Regular">
            {l.label}
          </SvgText>
        ))}
        {ghostLines.map((s, i) => (
      ...
      {locked && (
        <TouchableOpacity
          ...
          activeOpacity={0.9}
        >
          <View style={[fcStyles.chartLockBadge, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
            <Feather name="lock" size={13} color={theme.accent} />
...
function DCAEscalationSlider({
  ...
  return (
    <View style={[fcStyles.subCard, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
      <View style={fcStyles.sliderHeader}>
        ...
        <Text style={[fcStyles.sliderValueText, { color: theme.accent }]}>
      ...
      <View style={{ height: THUMB, justifyContent: "center", marginTop: 10, marginBottom: 4 }}
        ...
        {...pan.panHandlers}
      >
        <View style={[fcStyles.sliderTrack, { backgroundColor: theme.hairline }]} />
        <View style={[fcStyles.sliderFill,  { width: `${fillPct}%` as any, backgroundColor: theme.accent }]} />
        <View style={[fcStyles.sliderThumb, { left: `${fillPct}%` as any, marginLeft: -(THUMB / 2), backgroundColor: theme.accent }]} />
      </View>
      <View style={fcStyles.sliderTicks}>
        {[0, 2, 5, 7, 10].map(v => (
          <TouchableOpacity key={v} onPress={() => onChange(v)}
            hitSlop={{ top: 8, bottom: 8, left: 6, right: 6 }}>
            <Text style={[fcStyles.sliderTick, { color: v === value ? theme.accent : theme.textMuted }]}>
              {v}%
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {value > 0 && (
        <Text style={[fcStyles.subText, { color: theme.textMuted, marginTop: 8 }]}>
    ...
    <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
      <View style={fcStyles.titleRow}>
        ...
        {canUseWealthProjections && (
          <View style={[fcStyles.tierBadge, { backgroundColor: theme.accent + "22" }]}>
            <Text style={[fcStyles.tierBadgeText, { color: theme.accent }]}>PRO</Text>
          </View>
        )}
      ...
      <View style={[fcStyles.subCard, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
        <View style={fcStyles.inputRow}>
          ...
          <TextInput
            style={[fcStyles.input, { color: theme.text, borderColor: theme.hairline, backgroundColor: theme.surface }]}
            value={monthlyDCA}
        ...
        <View style={fcStyles.inputRow}>
          ...
          <Text style={[fcStyles.inputValue, { color: theme.accent }]}>{formatEUR(start)}</Text>
        </View>
          ...
          {HORIZONS.map(h => (
            <TouchableOpacity key={h}
              style={[fcStyles.segBtn, {
                backgroundColor: years === h ? theme.accent : theme.surface,
                borderColor:     years === h ? theme.accent : theme.hairline,
              }]}
              onPress={() => setYears(h)}
            >
      ...
      <View style={[fcStyles.subCard, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
        ...
        <Text style={[fcStyles.bigNumber, { color: theme.accent }]}>{formatEUR(dca * 12 * years + start)}</Text>
      ...
      <View style={[fcStyles.subCard, { backgroundColor: theme.surface, borderColor: theme.hairline, padding: 0, overflow: "hidden" }]}>
        <View style={[fcStyles.tableRow, { borderBottomColor: theme.hairline, backgroundColor: theme.surface }]}>
          <Text style={[fcStyles.tableHeader, { color: theme.textMuted, flex: 1 }]}>Year</Text>
          {visibleScenarios.map(sc => (
            <Text key={sc.key} style={[fcStyles.tableHeader, { color: sc.color, flex: 2, textAlign: "right" }]}>{sc.label}</Text>
          ))}
        </View>
        {tableData.map(row => (
          <View key={row.years} style={[fcStyles.tableRow, { borderBottomColor: theme.hairline }]}>
      ...
      {canUseWealthProjections && (
        <>
          ...
          <Text style={[fcStyles.subText, { color: theme.textMuted, marginBottom: 10 }]}>
          ...
          {SCENARIOS.map(sc => (
            <View key={sc.key} style={[fcStyles.inputRow, { marginBottom: 10 }]}>
              ...
              <TextInput
                style={[fcStyles.input, { color: sc.color, borderColor: sc.color + "44", backgroundColor: theme.surface, width: 70 }]}
...
const fcStyles = StyleSheet.create({
  ...
  subCard:      { borderRadius: 0, borderWidth: 1, padding: 14, marginBottom: 12 },
  ...
  segBtn:    { flex: 1, paddingVertical: 8, borderRadius: 0, borderWidth: 1, alignItems: "center" },
  ...
  sliderThumb: { position: "absolute", width: 22, height: 22, borderRadius: 0, top: -8 },
  ...
  return (
    ...
    >
            ...
            {PERF_RANGES.map((r) => (
              <TouchableOpacity
                ...
                style={[
                  styles.rangeBtn,
                  {
                    backgroundColor: selectedRange === r ? theme.accent + "22" : "transparent",
                    borderColor: selectedRange === r ? theme.accent : "transparent",
                  },
                ]}
                onPress={() => setSelectedRange(r)}
              >
                <Text style={[styles.rangeBtnText, { color: selectedRange === r ? theme.accent : theme.textTertiary }]}>
                  {r}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      <View style={styles.metricsGrid}>
        <View style={[styles.metricCard, { backgroundColor: theme.surface, borderColor: theme.hairline }]}>
...
const styles = StyleSheet.create({
  ...
  fakeLine: { position: "absolute", height: 2, borderRadius: 0, left: 0 },
  premiumOverlay: {
    position: "absolute",
    top: 30,
    left: 0,
